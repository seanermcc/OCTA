...
**Saved CNV overlays in octa-seg_v3 (2026-09-22):** the manual layer reviewer now loads the final v9 CNV dataset and newer saved final corrections. CNVs appear as pink outlines with translucent en-face fill and matching full-depth bands on the selected B-scan. Pending corrections remain identified as pending. Save and reopen an existing window to load the update. See the [reviewer guide](outputs/octa-seg/octa-seg_v3/review/START_HERE.md).

**Final CNV correction (2026-09-22):** [Open the final correction editor](outputs/octa-auto_cnv_v9/OPEN_FINAL_CNV_CORRECTION.cmd). The last assessment flagged 15 entries across 14 acquisitions; this separate queue starts from their exact selected footprints and preserves both references for the duplicated acquisition. The user's acceptance of the rest is recorded: 203 positive acquisitions, 105 confirmed negatives and two retained poor-image exclusions. After editing, **Export final dataset + sizes** consolidates the cohort to one record per acquisition and reports pending cases. See the [review and export guide](outputs/octa-auto_cnv_v9/final_correction/START_HERE.md).

**octa-reg_v3 (2026-09-22):** [Open the new montage reviewer](http://127.0.0.1:8774/) or launch `outputs\octa-reg_v3\OPEN_REVIEWER.cmd`. All 324 scans in 19 eyes are accounted for after 23,864 candidate-pose trials using large vessels, fine detail and selected v9 CNV locations. Saved TS165 OD/OS placements, individual confirmations, flags and the manual ONH origin are preserved; the other 17 eyes were rerun. Results are 112 supported, 209 flagged, one unlocalized and two source exclusions. The TS165 development comparison improved OD but not OS consistently, so this is not a claim of improved cohort accuracy. Pink CNV outlines can be toggled. v2 and source annotations remain unchanged. See the [run report](outputs/octa-reg_v3/REPORT.md) and [review guide](outputs/octa-reg_v3/REVIEW_GUIDE.md).

**Final CNV assessment (2026-09-22):** [Open the Structural OCT assessment](http://127.0.0.1:8805/), or launch `outputs\octa-auto_cnv_v9\OPEN_FINAL_CNV_ASSESSMENT.cmd`. It retains 73 frozen manual training references, 10 confirmed Model 2 results and 78 confirmed Model 3 results, and adds all 57 confirmed Model 3 corrections: 218 entries across 217 acquisitions. A separate view contains 105 confirmed no-CNV samples and two deferred/poor-image samples. Unflagged CNV entries default to good; **Confirm all samples** records approval for the entire CNV group while preserving review flags and notes. New assessments save separately. See the [selection and saving guide](outputs/octa-auto_cnv_v9/final_assessment/START_HERE.md).

**Montage placement review (2026-09-21):** all 19 cohort reviewers now open with flagged proposals visible and support day filters, individual field and whole-montage confirmation, drag/rotate corrections, undo/reset, and automatic disk saving with revision history. Launch `outputs\octa-reg_v2\all_samples\OPEN_REVIEWER.cmd`; see the [review guide](outputs/octa-reg_v2/all_samples/REVIEW_GUIDE.md). Anatomical NSEW remains unconfirmed: coordinates retain the ONH/reference image axes. Original coverage totals and PNGs do not incorporate manual corrections.

**Full-cohort octa-reg_v2 montages (2026-09-21):** [Open the cohort reviewer](outputs/octa-reg_v2/all_samples/index.html), or launch `outputs\octa-reg_v2\all_samples\OPEN_REVIEWER.cmd`. All 324 scans are accounted for in 19 separate animal-eye groups, pooling dates: 167 supported placements, 154 flagged proposals, one unlocalized scan and two preserved exclusions. The reviewer independently toggles supported and flagged fields and reports their coverage separately. The reusable `octa-reg_v2` skill is installed in `C:/Users/seane/.codex/skills/octa-reg_v2`; its [archived instructions](outputs/octa-reg_v2/skill/octa-reg_v2/SKILL.md) document reproduction. See the [cohort report](outputs/octa-reg_v2/all_samples/REPORT.md) for evidence and limitations; automatic placements still require review.

**TS247 OD pooled retinal montage (2026-09-21):** the exploratory [octa-reg_v2 viewer](outputs/octa-reg_v2/TS247_OD/index.html) places 33 eligible scans around a reviewed ONH, pooling dates at the user's request. Nine representative fields show approximately 4.31 single-field areas of coverage; all 33 placements are inspectable. One prior exclusion is retained. See the [run report](outputs/octa-reg_v2/TS247_OD/REPORT.md) for the low-contrast exception, registration evidence and limitations. The frozen v1 run is unchanged.

**CNV v8 Model 1 correction queue (2026-09-18):** the conservative model's exact
30-acquisition preview set is ready in an adapted copy of the latest v7 editor.
Open [OPEN_MODEL1_CORRECTION.cmd](outputs/octa-auto_cnv_v8/OPEN_MODEL1_CORRECTION.cmd).
Its 100 initial candidate regions require human review; saved corrections and
whole-image confirmations feed per-lesion areas, per-scan union areas and
confirmed size distributions. See the [review guide](outputs/octa-auto_cnv_v8/manual_review/START_HERE.md)
and [next-model plan](outputs/octa-auto_cnv_v8/manual_review/TRAINING_PLAN.md).
Review records are isolated from v7 and frozen v8 predictions; training is pending review.
