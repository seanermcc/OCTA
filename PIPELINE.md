...
**Learned boundary cost.** The intended next step, and deliberately not a
U-Net. `build_costs()` → `SURFACE_COST` → `banded_dp` already isolates the one
...
**Learned segmenter (U-Net).** Higher ceiling, standard in the OCT literature,
needs torch and realistically several hundred labelled B-scans. Revisit once
the learned cost model is working and the labelling rate is known.
