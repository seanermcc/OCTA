...
DEFAULT_ROOT = Path(__file__).resolve().parents[2] / 'outputs/octa-reg_v2/all_samples'
LOCK = threading.Lock()
...
def current(folder, data):
    ...
    if path.exists():
        ...
        return result
    return dict(schema='octa-reg-v2-human-review-1', group=data['summary']['group'],
...
def save(folder, data, body):
    ...
    result = dict(schema='octa-reg-v2-human-review-3', group=data['summary']['group'],
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/api/health': return self.reply(200, {'service': 'octa-reg-v2-review', 'supports_manual_onh': True})
