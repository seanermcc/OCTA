...
# QSettings(org, app) can still select the Windows native backend despite
# setDefaultFormat. Use an explicit file to isolate all GUI preferences.
settings_constructor=QtCore.QSettings
settings_patch=patch.object(QtCore,'QSettings',side_effect=lambda *a,**kw:
    settings_constructor(str(folder/'preferences.ini'),settings_constructor.Format.IniFormat))
settings_patch.start()
errors=[]
def error_hook(typ,exc,tb):
    import traceback
...
(root/'PORTABLE_REAL_VALIDATION.json').write_text(json.dumps(dict(
    status='passed',volumes=len(entries),confirmed_cases=results,external_source_access_attempts=denied,
    gui_errors=errors),indent=2),encoding='utf-8')
