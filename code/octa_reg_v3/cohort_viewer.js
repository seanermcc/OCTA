...
$('#title').textContent=S.group.replace('_',' ')+' · Retinal montage';document.title=S.group+' · octa-reg_v3';
...
$('#origin').textContent=S.origin_kind==='manual_onh'?'Origin: carried manual ONH':S.origin_kind==='reviewed_onh'?'Origin: reviewed visible ONH':S.origin_kind==='estimated_onh'?'Origin: estimated ONH':'ONH unresolved · reference-field coordinates';
...
for(const s of scans){
  ...
  if(s.cnv_image){const overlay=new Image();overlay.onload=draw;overlay.src=s.cnv_image;cnvImages.set(s.index,overlay);}
...
function status(){
  ...
  const originText=review.onh_override!==null?'Origin: manually placed ONH':S.origin_kind==='manual_onh'?'Origin: carried manual ONH':S.origin_kind==='reviewed_onh'?'Origin: reviewed visible ONH':S.origin_kind==='estimated_onh'?'Origin: estimated ONH':'ONH unresolved · reference-field coordinates';
  ...
  $('#coverageNote').textContent=`Release coverage: supported ${S.supported_coverage_mm2.toFixed(2)} mm²; with proposals ${S.proposed_coverage_mm2.toFixed(2)} mm². These totals are not recalculated for filters or manual edits.`;
...
function details(){
  ...
  if(s.cnv){const p=document.createElement('p');p.className='note';p.textContent='CNV: '+s.cnv.source.replaceAll('_',' ')+' · '+s.cnv.pixels+' px · '+(s.cnv.complete?'whole-field confirmed':'unconfirmed / partial')+'. Pink outlines are supporting evidence.';$('#detail').append(p);}
...
function field(index,opacity=1){
  ...
  ctx.globalAlpha=opacity;ctx.drawImage(im,5,5,502,502,5,5,502,502);const overlay=cnvImages.get(index);if($('#cnvToggle').checked && overlay?.complete && overlay.naturalWidth)ctx.drawImage(overlay,0,0);ctx.globalAlpha=1;
...
function exportPayload(){
  ...
  return {...review,schema:'octa-reg-v3-human-review-1',group:S.group,base_fingerprint:baseFingerprint,revision,unsaved:dirty||saving,analysis_records:rows,effective_matrices_to_onh_pixels:effective,inherited_from:window.MONTAGE.inherited_review?{source:window.MONTAGE.inherited_review.source,revision:window.MONTAGE.inherited_review.revision}:null,coordinate_system:'fields use release frame; effective matrices use current ONH/reference origin',analysis_policy:'Confirmed montage required; supported only by default; flagged opt-in; excluded never included.'};
}
