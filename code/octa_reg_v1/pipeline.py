...
from .geometry import features,register,fit_onh,branches,convergence,build_components,onh_fit
...
def prepare(row,inventory,records,export,cfg,pin):
    ...
    info['visible_onh']=onh_fit(arrays,info,cfg)
    ...
    if bad:
        ...
        try:
            ...
            if min(len(fa['points']),len(fb['points']))>=cfg['registration_min_matches']:
                ...
                if result['matrix'] is None and len(mm)>=cfg['registration_min_matches']:
                    # The inherited early-return omits failed-consensus diagnostics.
                    # Repeat the identical deterministic fit for reporting only; never promote it.
                    import warnings
                    ...
                    if model is not None and bool(model) and inliers is not None:
                        result['inliers']=int(inliers.sum())
                        result['failed_consensus_matrix']=model.params.tolist()
                        result['residual_um']=float(np.sqrt(np.mean(model.residuals(pa[inliers],pb[inliers])**2))) if inliers.any() else None
...
def run(out,inventory,export,cfg,eye=None):
    ...
    try:
            ...
            def safe_prepare(row):
                try:return prepare(row,inventory,records,export,cfg,pin)
                    ...
                    return dict(row,status='blocked',error=type(e).__name__+': '+str(e),traceback=traceback.format_exc(),
                        failure_class='input_unavailable_or_invalid' if isinstance(e,(FileNotFoundError,ValueError)) else 'implementation_failure',
        ...
        final='failed' if changed or counts['implementation_failure'] or any(i.get('failure_class')=='implementation_failure' for i in infos.values()) else 'complete-with-blocked-inputs' if any(i['status']=='blocked' for i in infos.values()) else 'complete'
