...
def save_label(
    ...
    *,
    ...
    editor_provenance: dict | None = None,
) -> Path:
    ...
    if editor_provenance:
        with np.load(temp, allow_pickle=False) as stored:
            payload = {key: stored[key].copy() for key in stored.files}
        if set(payload) & set(editor_provenance):
            raise ValueError('Editor provenance must not replace standard label fields')
        payload.update(editor_provenance)
