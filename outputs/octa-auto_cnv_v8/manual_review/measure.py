...
def run(directory=None,output=None,synthetic=False):
    ...
    for a in queue:
        ...
        base={k:a[k] for k in ('scan_id','animal','eye','session_date','day_label','days_post_laser','preview_role','model1_training_exposure')}
            ...
            try:
                ...
                if valid_confirmation(store.state):
                    ...
                    training.append(dict(**base,revision=store.revision,label=fingerprint(path),targets=fingerprint(target_path),
                        source_identity=a['source_identity'],model1_provenance=a['model1_provenance'],
    ...
    basecols=['scan_id','animal','eye','session_date','day_label','days_post_laser','preview_role','model1_training_exposure']
