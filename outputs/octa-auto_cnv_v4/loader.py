...
from thickness import load_thickness, review_config
...
class ProposalLoader(gui.Loader):
    def run(self):
        try:
            ...
            layer_values, layer_metadata = load_thickness(sid)
            a['layer_thickness_um'] = layer_values
            meta = json.loads(str(a['metadata_json']))
            meta['layer_thickness'] = layer_metadata
            out = volume_path(sid)
            ...
            index = SurfaceIndex([HERE/'review/surface_labels', *config['manual_sources']])
            index.refresh(scan)
