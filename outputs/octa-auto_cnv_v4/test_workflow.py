...
class GuiTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        a['layer_thickness_um']=measure(rows,np.ones_like(rows),a['shadow'])[0]
    ...
    def test_closed_paint_fills_and_open_paint_does_not(self):
        w=self.window;w.auto_check.setChecked(False);w.diameter.setValue(2);w.brush('paint')
        w._outline_drawn([(3,2),(11,2),(11,9),(3,9),(3,2)],'cnv_brush_add')
        self.assertTrue(w.selected_region().mask[5,7]);self.assertTrue(w.selected_region().touched[5,7])
        self.assertFalse(w.selected_region().core.any());self.assertFalse(w.selected_region().complete)
        w.undo_region();self.assertEqual(len(w.store.regions),1);w.redo_region()
        w.approve();w.save_all()
        re=ReviewStore(self.store.path.parent,self.scan,self.scan.maps['core'],str(self.seed))
        self.assertTrue(re.regions[-1].mask[5,7]);self.assertTrue(decode_mask(re.regions[-1].record()['reviewed_runs'],self.scan.native_shape)[5,7])
        w.select_region(-1);w.brush('paint');w._outline_drawn([(20,2),(28,2),(28,9)],'cnv_brush_add')
        self.assertFalse(w.selected_region().mask[5,24])

    def test_actual_mouse_circle_fills_on_all_three_panels(self):
        from PySide6.QtTest import QTest
        from viewer import Qt,gui
        w=self.window;w.auto_check.setChecked(False);w.diameter.setValue(2)
        for canvas in (w.structural,w.evidence,w.octa):
            w.select_region(-1);w.brush('paint');canvas.fit()
            points=[canvas.mapFromScene(gui.QtCore.QPointF(x,y)) for x,y in [(3,2),(11,2),(11,9),(3,9),(3,2)]]
            ...
            self.assertTrue(w.selected_region().mask[5,7])

    def test_hidden_automatic_selection_cleared_on_scan_load(self):
        w=self.window;w.auto_check.setChecked(False)
        blank=np.zeros(self.scan.native_shape,bool)
        w.pending_index=0
        w._loaded((self.scan,self.scan.surfaces,self.scan.confidence,dict(name='fixture',path=''),
            (blank,blank,blank,blank,dict(status='fixture',path=''),''),w.index,self.store))
        self.assertEqual(w.selected,-1)

    def test_circle_across_strokes_fills_but_preserves_later_erasure(self):
        w=self.window;w.auto_check.setChecked(False);w.diameter.setValue(2);w.brush('paint')
        w._outline_drawn([(3,2),(11,2),(11,9)],'cnv_brush_add')
        self.assertFalse(w.selected_region().mask[5,7])
        w._outline_drawn([(11,9),(3,9),(3,2)],'cnv_brush_add')
        self.assertTrue(w.selected_region().mask[5,7])
        w.brush('erase');w._outline_drawn([(7,5)],'cnv_brush_add')
        self.assertFalse(w.selected_region().mask[5,7])
        w.brush('paint');w._outline_drawn([(13,8)],'cnv_brush_add')
        self.assertFalse(w.selected_region().mask[5,7])
        w._outline_drawn([(3,2),(11,2),(11,9),(3,9),(3,2)],'cnv_brush_add')
        self.assertTrue(w.selected_region().mask[5,7])

    def test_layer_switch_leaves_full_retina_and_review_unchanged(self):
        w=self.window;before=self.store.signature();full=w.displayed_full.copy()
        for i in range(7):
            w.layer_choice.setCurrentIndex(i)
            self.assertTrue(np.array_equal(w.displayed_layer,self.scan.maps['layer_thickness_um'][i+1],equal_nan=True))
            self.assertTrue(np.array_equal(w.displayed_full,full,equal_nan=True))
            self.assertIn('Full retina',w.octa.parentWidget().title())
        self.assertEqual(before,self.store.signature());self.assertFalse(self.store.path.exists())
        w.step_layer(1);self.assertEqual(w.layer_choice.currentIndex(),0)
        self.scan.maps['layer_thickness_um'][1]=np.nan;w.render_maps()
        self.assertIn('unavailable',w.evidence.parentWidget().title())
        self.assertTrue(np.isfinite(w.displayed_full).any())

    def test_v3_review_is_loaded_read_only_and_edits_save_in_v4(self):
        w=self.window;w.approve();w.save_all();original=self.store.path;digest=sha(original)
        nextdir=self.root/'v4/regions'
        carried=ReviewStore(nextdir,self.scan,self.scan.maps['core'],str(self.seed),previous_directory=original.parent)
        self.assertEqual(carried.regions[0].decision,'approved');self.assertFalse(carried.dirty)
        carried.save({},[],{});self.assertFalse(carried.path.exists())
        carried.regions[0].notes='v4 edit';carried.save({},[],{})
        self.assertTrue(carried.path.exists());self.assertEqual(sha(original),digest)
        self.assertEqual(carried.regions[0].events[-1]['sha256'],digest)
