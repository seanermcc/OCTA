...
REVIEW_POLICY = 'review-warnings-1'
...
def review_warnings(unresolved, lesion):
    """Exact review warnings acknowledged by the user; edge and region are independent."""
    warnings = {}
    if unresolved.any():
...
def resolve(events, baseline, offset, depth):
    ...
    shadow_sources = np.zeros((n + 1, width), bool)  # Eight retinal boundaries + CNV edge.
    ...
    for revision, event in enumerate(events):
        ...
        if action == 'confirm_bscan':
            ...
            policy = event.get('review_policy')
            if policy not in (None, REVIEW_POLICY):
                raise ValueError('Unknown confirmation review policy')
            if policy == REVIEW_POLICY and event.get('acknowledged_warnings') != review_warnings(problems, lesion):
                raise ValueError('Confirmation warning acknowledgment differs from the current review')
            if (event.get('state_digest') != state_digest(current) or event.get('scope') != [n, width]
                    or (problems.any() and policy is None) or event.get('geometry_revision') != geometry_revision):
            ...
            lesion_confirmation = None
            if event.get('lesion_contract') is not None:
                version = event.get('lesion_definition')
        ...
        if event.get('semantics', 1) >= 2:
            ...
            lesion_confirmation = None
        geometry_revision = event.get('id', 'legacy-' + str(revision))
        if action in L.ACTIONS:
            version = event.get('lesion_definition')
            if lesion_definition is not None and version != lesion_definition:
                raise ValueError('Mixed lesion definitions require explicit migration, not silent relabeling')
            L.apply(lesion, event, offset)
            if action == 'cnv_edge':
                shadow_sources[n, int(event['lo']):int(event['hi'])] = not event.get('erase', False) and not event.get('unreliable', False)
            elif action == 'cnv_edge_mark' and event['mark'] in ('absent', 'reset_boundary'):
                shadow_sources[n, int(event['lo']):int(event['hi'])] = False
            lesion_definition = version
            strokes += 1
            continue
        ...
        if action == 'stroke':
            ...
            if drawing_reliability is not None:
                ...
                shadow_sources[k, support] = False
                shadow_sources[k, direct] = drawing_reliability == 'reliable'
            # A drawn best guess does not silently clear an anatomical-absence judgment.
            approved[k, support] = False
            strokes += 1
            continue
        for k in ks:
            if action == 'erase_boundary':
                # Erasure removes geometry, not anatomy/traceability/reliability judgments.
                z[k, span] = np.nan
                shadow_sources[k, span] = False
                for plane in (drawn, taper, displaced, reviewed, approved, display_reliable):
                    plane[k, span] = False
                continue
    ...
    if not had_confirmation:
        approved &= ~displaced
    # Only surviving explicitly reliable strokes lift the automatic shadow veto.
    # Joins, confirmation, region painting and automatic neighbor movement do not.
    shadow_sources[:n] &= drawn & ~displaced & (trace == 1) & (reliability == 1) & (anatomy != 0) & np.isfinite(z)
    shadow_sources[n] &= (lesion['cnv_edge_state'] == 1) & np.isfinite(lesion['cnv_edge'])
    result = dict(positions=z, trace=trace, reliability=reliability, anatomy=anatomy,
                ...
                approved=approved, display_reliable=display_reliable, shadow_override_sources=shadow_sources,
                ...
                lesions=lesion, lesion_confirmation=lesion_confirmation,
...
def position_targets(resolved, shadow, valid_geometry, vessel=None, include_shadow_overrides=True):
    ...
    guarded = boundary_uncertainty(r['positions'].shape, vessel, shadow)
    ...
    approved &= ~r['excluded'][None] & ~np.asarray(shadow)[None] & valid_geometry & ~guarded
    if r.get('confirmation'):
        trace[approved] = reliability[approved] = 1
    # Preserve human judgments in the journal; a context veto is not a new manual label.
    reliability[guarded & (reliability == 1)] = -1
    return dict(reliable_manual=eligible & (r['reliability'] == 1) & ~guarded,
                ambiguous_manual=eligible & (r['reliability'] != 1),
                approved_position=approved,
                trace=trace, reliability=reliability, anatomy=r['anatomy'].copy(), context_uncertain=guarded,
                human_reliability=r['reliability'].copy())


def training_targets(record, baseline, offset, depth, shadow, role='development', vessel=None, include_shadow_overrides=True):
    ...
    targets = position_targets(r, shadow, r['valid_geometry'], vessel, include_shadow_overrides)
    effective_shadow, overridden = shadow_masks(r, shadow, include_shadow_overrides)
    ...
    targets['ambiguous_candidate'] &= np.isfinite(r['positions']) & ~r['excluded'][None] & ~effective_shadow[None] & eligible
    ...
    targets.update(vessel_mask=np.zeros_like(shadow, dtype=bool) if vessel is None else np.asarray(vessel, bool).copy(),
                   ...
                   shadow_override_sources=r['shadow_override_sources'] & np.asarray(shadow, bool)[None])
    ...
    targets.update(lesion_eligible=lesion_eligible,
                   lesion_definition=r['lesion_confirmation']['lesion_definition'] if lesion_eligible else None,
