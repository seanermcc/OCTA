...
class Editor(BoundaryEditor):
    ...
    def set_volume_line(self, volume, row, pinned_role=None):
        ...
        row_height = self.surface_list.sizeHintForRow(0)
        if row_height > 0:
            self.surface_list.setFixedHeight(row_height * self.n_boundaries + 2 * self.surface_list.frameWidth())
    ...
    def recompute(self, resolved=None):
        ...
        self.rendered = render(base, r, self.offset, self.pack.images.shape[1], self.volume.data['shadow'][self.row],
                               vessel_columns(self.volume, self.row))
        ...
        boundary_events = [e for e in self.journal.events if e['action'] != 'case_metadata' and e['action'] not in L.ACTIONS]
    ...
    def record_event(self, action, lo=0, hi=None, boundaries=None, **payload):
        ...
        if action == 'stroke' or action in L.ACTIONS:
    ...
    def action(self, action):
        ...
        if hi <= lo:
            ...
            return
        mode = self.lesion_tools.mode
        if mode is not None and action not in ('exclude_image', 'clear_exclusion', 'unreliable_region', 'clear_region'):
    ...
    def eventFilter(self, obj, event):
        if self.read_only and obj in (self.canvas, self.canvas.viewport()) and event.type() in (
            ...
            if event.buttons() & (Qt.MouseButton.LeftButton | Qt.MouseButton.RightButton) or (
                ...
                return True
        if hasattr(self, 'lesion_tools') and self.lesion_tools.handle_event(obj, event):
            return True
        ...
        if (obj == self.canvas.viewport() and self.journal is not None
                ...
                and not self.canvas._space and self.lesion_tools.mode is None and self.auto_pick.isChecked()):
    ...
    def redraw_surfaces(self):
        ...
        sync_all_boundaries(self)
        # Replace the inherited red context with the v3 pink footprint below.
        lesion, self._lesion = self._lesion, None
        try:
        ...
        if self._lesion is not None:
            ...
            depth = self.pack.images.shape[1]
            for lo, hi in L.runs(self._lesion):
                path.addRect(lo - .5, 0, hi - lo, depth)
                for x in (lo - .5, hi - .5):
                    borders.moveTo(x, 0); borders.lineTo(x, depth)
            for shape, outline in ((path, False), (borders, True)):
                color = QtGui.QColor(PINK)
                color.setAlpha(155 if outline else 30)
                pen = QtGui.QPen(color, 1) if outline else QtGui.QPen(Qt.PenStyle.NoPen)
                pen.setCosmetic(True)
                brush = QtGui.QBrush(Qt.BrushStyle.NoBrush) if outline else QtGui.QBrush(color)
                item = self.canvas.scene().addPath(shape, pen, brush)
                item.setZValue(4)
        ...
        for mask, overridden in ((self.rendered['effective_shadow_mask'], False),
                                 (self.rendered['shadow_override_columns'], True)):
            path = QtGui.QPainterPath()
            for lo, hi in L.runs(mask):
                path.addRect(lo-.5, 21 if overridden else 0, hi-lo,
                             4 if overridden else self.pack.images.shape[1])
            tint = QtGui.QColor('#ffad55')
            tint.setAlpha(150 if overridden else 32)
            item = self.canvas.scene().addPath(path, QtGui.QPen(Qt.PenStyle.NoPen), QtGui.QBrush(tint))
            item.setZValue(40 if overridden else 3)
            item.setAcceptedMouseButtons(Qt.MouseButton.NoButton)
            item.setData(0, 'shadow_override_marker' if overridden else 'automatic_shadow_tint')
            item.setToolTip('Manual shadow override: a reliable stroke lifted the automatic shadow mask for all boundaries in these columns. Kept as a separate analysis/training group.' if overridden else
                            'Automatic shadow mask (not a manual unreliable judgment). A normal reliable boundary stroke lifts this mask for all boundaries in the drawn columns.')
            self._extras.append(item)
        ...
        for k in range(self.n_boundaries):
            ...
            shown = self.surface_list.item(k).checkState() == Qt.CheckState.Checked
            shown &= not self.only_active.isChecked() or k == self.s
            if shown and bad.any():
                pen = QtGui.QPen(QtGui.QColor('#ff3030'), 5); pen.setCosmetic(True)
                yy = np.full(self.width, 3 + k * 5, dtype=float)
                overlay = self.canvas.scene().addPath(curve_path(yy, bad), pen)
                overlay.setToolTip(f'{self.pack.names[k]}: unresolved positions (missing, out-of-image or crossing); correct, mark an exception or acknowledge when confirming')
                overlay.setData(0, 'unresolved_boundary')
                overlay.setData(1, k)
                overlay.setZValue(45); self._extras.append(overlay)
        if self.surface_list.item(self.s).checkState() != Qt.CheckState.Checked:
            for item in [*self.canvas._band_items.values(), *self.canvas._ruler_items.values()]:
                item.setPath(QtGui.QPainterPath())
        self.lesion_tools.render()
    ...
    def commit_current(self):
        ...
        if not any(e['action'] != 'case_metadata' and e['action'] not in L.ACTIONS for e in self.journal.events):
            return
    ...
    def keyPressEvent(self, event):
        ...
        if ctrl and key in (Qt.Key.Key_Z, Qt.Key.Key_Y):
            ...
            if self.lesion_tools.mode:
                self.action('clear_marks')
            else:
                self.record_event('clear_marks', 0, self.width)
        elif key == Qt.Key.Key_E:
            self.lesion_tools.erase.toggle()
        else:
