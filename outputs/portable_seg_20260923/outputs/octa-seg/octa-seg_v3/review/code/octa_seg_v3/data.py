...
def local_path(value):
    ...
    if portable.enabled():
        return portable.local_path(value)
    path = Path(value)
    if path.exists():
        return path
...
def load_volume(entry, image_budget=1024**3):
    ...
    # Saved reviews/queues retain their original absolute provider path. Resolve
    # a relocated project just as we do its images/source, without editing the
    # journal or substituting another scan/provider. Model hashes still apply.
    path = local_path(entry['directory'])
    entry = dict(entry, directory=str(path))
    prep = read(path / 'prepared.json')
    from . import portable
    portable_source = portable.validate_scan(entry['scan_id'], path, prep) if portable.enabled() else None
    ...
    source = portable_source if portable_source is not None else local_path(prep['source']['path'])
    ...
    overlays = portable.overlays(scan.scan_id) if portable.enabled() else load_enface(scan, ROOT / 'outputs/cnv_labels', proposal_dir)
