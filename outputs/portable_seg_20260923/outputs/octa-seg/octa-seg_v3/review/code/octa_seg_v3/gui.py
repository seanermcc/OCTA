...
class Navigator(ReviewCanvas):
    def set_annotations(self, cnv, vessel=None, onh=None, edge=None):
        from scipy.ndimage import binary_erosion
        from .cnv_context import PINK
        # Keep existing vessel/ONH colors, then draw CNV fill and exact pixel edge.
        super().set_annotations(np.zeros_like(cnv), vessel, onh, edge)
        mask = np.asarray(cnv, bool)
        rgba = np.zeros((*mask.shape, 4), np.uint8)
        color = QtGui.QColor(PINK)
        rgba[mask] = (color.red(), color.green(), color.blue(), 30)
        rgba[mask & ~binary_erosion(mask)] = (color.red(), color.green(), color.blue(), 240)
        image = QtGui.QImage(rgba.data, mask.shape[1], mask.shape[0], rgba.strides[0],
    ...
    def review_lines(self, records):
        ...
        width = self._shape[1]
        for r in records:
            pen = QtGui.QPen(QtGui.QColor('#41e36f' if r['status'] == 'Confirmed' else '#ffea00'), 2.7,
        ...
        pen = QtGui.QPen(QtGui.QColor('#ff9600'), 3, Qt.PenStyle.SolidLine)
...
class Window(QtWidgets.QMainWindow):
    ...
    def __init__(self, reviewer='lead', queue_path=None, entries=None, output=None, autoload=True, cache=None, queue_output=None, read_only=False):
        ...
        if portable.enabled():
            self.setWindowTitle(self.windowTitle() + ' · PORTABLE CACHE · ' + str(OUT.parents[3]))
    ...
    def build_left(self):
        ...
        self.coverage_hint = QtWidgets.QLabel('Measurements only; uncertain, absent and shadowed locations stay blank. Scale stays fixed while editing.')
        self.coverage_info = info_button(self.coverage_hint, 'About thickness coverage')
        self.coverage_info.hide()
        layout.addWidget(self.map_scale)
        self.coverage_line = QtWidgets.QWidget()
        coverage_row = QtWidgets.QHBoxLayout(self.coverage_line)
        coverage_row.setContentsMargins(0, 0, 0, 0)
        self.coverage_label = QtWidgets.QLabel('')
        coverage_row.addWidget(self.coverage_label)
        coverage_row.addWidget(self.coverage_info)
        coverage_row.addStretch()
        layout.addWidget(self.coverage_line)
        self.coverage_line.hide()
        layout.addWidget(self.coverage_hint)
    ...
    def install_volume(self, volume, index, row, cached):
        ...
        self._shadow_overrides = np.zeros_like(volume.data['shadow'], dtype=bool)
        ...
        for entry in saved_index(self.reviewer, self.output):
            ...
            out = render({k: a[b] for k, a in volume.base.items()}, r, int(volume.data['label_offset']),
                         volume.images.shape[1], volume.data['shadow'][b], vessel_columns(volume, b))
            self._maps[b] = out['thickness_um']
            self._shadow_overrides[b] = out['shadow_override_columns']
    ...
    def on_editor_changed(self):
        ...
        self._shadow_overrides[self.row] = self.editor.rendered['shadow_override_columns']
    ...
    def update_map(self):
        ...
        if self.map_tabs.currentIndex() == 0:
            ...
            if key not in THICKNESS_NAMES:
                return
            values = np.where(vessel_columns(self.volume) | (self.volume.data['shadow'] & ~self._shadow_overrides), np.nan,
                              self._maps[:, THICKNESS_NAMES.index(key), :])
            ...
            self.map_scale.setText(f'{limits[0]:.1f} – {limits[1]:.1f} µm')
            self.coverage_label.setText(f'{np.isfinite(values).mean():.0%} coverage')
            self.coverage_line.show()
            self.coverage_info.show()
            self.coverage_hint.setVisible(self.coverage_info.isChecked())
            from matplotlib import colormaps
    ...
    def update_point(self):
        ...
        if self.map_tabs.currentIndex() == 1 and self.map_choice.currentData() in THICKNESS_NAMES:
            ...
            if vessel_columns(self.volume, self.row)[self.col] or (self.volume.data['shadow'][self.row, self.col] and not self._shadow_overrides[self.row, self.col]):
                value = np.nan
    ...
    def refresh_overlays(self):
        ...
        self.editor.recompute()  # Refresh derived uncertainty without writing annotation.
        self.update_map()
