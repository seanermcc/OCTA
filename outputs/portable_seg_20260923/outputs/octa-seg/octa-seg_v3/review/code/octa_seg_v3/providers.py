...
def context_signature(sid):
    ...
    if portable.enabled():
        return portable.signature(sid)
    from .cnv_context import signature_paths
    paths = list(manifests())
    paths += signature_paths(sid)
...
def saved_cnv(volume):
    from .cnv_context import load_final_cnv
    final = load_final_cnv(volume)
...
def context_overlays(volume, proposal_dir, experimental_onh=False):
    ...
    if portable.enabled():
        if experimental_onh:
            raise ValueError('Portable package contains frozen default overlays; experimental ONH is not included.')
        return portable.overlays(volume.scan.scan_id)
    scan = volume.scan
    ...
    try:
        ...
        # Do not resurrect an older lesion when the current authoritative source
        # is unavailable or incompatible. Other context remains usable.
        errors.append('CNV: ' + str(exc))
        current_cnv = (np.zeros(scan.native_shape, bool), dict(status='CNV context unavailable; see error'))
    if current_cnv:
