...
def longitudinal(figs,d,out):
    ...
    for (animal,definition),g in normalized.groupby(['animal','region_definition']):
        ...
        eligible=g[g.identity_eligible]
        if eligible.empty: continue
        categorical=eligible.day.isna().any()
        visit=eligible[['visit_id','day','day_label','day_basis','session_date']].drop_duplicates().sort_values('session_date')
...
def onh_figures(figs,d,out):
    ...
    for ax,layer in zip(axes.flat,LAYERS):
        ...
        ax.set_xlabel('N* ← x (µm) → T*'); ax.set_ylabel('V* ← y (µm) → D*'); fig.colorbar(scatter,ax=ax,shrink=.7,label='µm')
