...
from .common import (ROOT,LAYERS,POLICY,ORIENTATION,read,write,sha,fingerprint,digest,
                     csvwrite,npz,save,default_config,validate_config,gate,day_info)
from . import annotations as A
from .geometry import Footprint,grid,regions,transform,rigid,recover_outline
...
class Runner:
    ...
    def inventory(self):
        ...
        self.status('inventory',**data['coverage'],measurement_gate='passed' if Path(self.c['gate']).exists() else 'waiting for FINAL_VERIFIED.json')
        return data
        ...
        for number,r in enumerate(inv['scans']):
            ...
            for definition,items in families:
                ...
                for basis in ('normalized','absolute_um'):
                    ...
                    measured=regions([v[1] for v in items],thick.shape[1:],a['spacing'],a['onh'],edges_for)
                    for (lid,f,ident,source),(bands,distances,diag) in zip(items,measured):
                        ...
                        if onh['state']=='localized':
                            ...
                            sector=('T*' if dx>0 else 'N*') if abs(dx)>abs(dy) else ('V*' if dy>0 else 'D*')
