...
from octa_reg_v3.warp_pilot import GRID,sample,feature

OUT=Path(__file__).resolve().parent
ROOT=OUT.parents[2]
BASE=OUT.parent/'bigwarp_pilot_20260922'
results=json.loads((OUT/'results.json').read_text())
for name in ('warp_pilot.py','test_warp_pilot.py'):
    shutil.copyfile(ROOT/'code/octa_reg_v3'/name,OUT/name)
def corr(a,b):return float(np.corrcoef(a,b)[0,1])
lines=['# Automatic residual-warp pilot: eight pairs', '',
       ...
       '| --- | --- | ---: | ---: |']
for r in results:
    baseline=r['models'][0];chosen=next(m for m in r['models'] if m['name']==r['selected'])
    a=baseline['audit'];b=chosen['audit']
    text=f'{a["median_px"]:.2f} → {b["median_px"]:.2f}' if a else 'not enough points'
    lines.append(f'| {r["group"]} | {r["selected"]} / {r["status"]} | {text} | {a["n"] if a else 0} |')
...
html=['<!doctype html><html><head><meta charset="utf-8"><title>Eight-pair warp pilot</title><style>body{font:16px system-ui;background:#17232b;color:#eef5f8;margin:24px;max-width:1500px}a{color:#8ee3f2}button{font:inherit;padding:8px 16px;cursor:pointer}section{border-top:1px solid #567;margin-top:28px;padding-top:12px}.pair{display:flex;gap:12px}.pair>div{width:49%}img{width:100%;max-width:650px}p{max-width:1050px}.badge{color:#ffc879}</style></head><body><h1>Automatic warp pilot · eight pairs</h1><p>Two provisional improvements; six pairs kept unchanged. Your original montages are preserved. These are pairwise experiments, not corrected full montages.</p><p>Red = moving, cyan = target. Matching structure appears gray/black. Inspect vessel branches and bends; brightness differences alone are not geometric errors.</p><p><a href="REPORT.md">Measurements, limitations and Fiji instructions</a> · <a href="#TS241_OS">Start Fiji review: TS241 OS</a></p>']
metrics=[]
for r in results:
    g=r['group'];folder=OUT/g;src=BASE/g
    for source,dest in [('moving_manual_aligned.tif','manual_moving.tif'),('target.tif','target.tif')]:shutil.copyfile(src/source,folder/dest)
    a=np.array(Image.open(folder/'manual_moving.tif'),float)/255;b=np.array(Image.open(folder/'target.tif'),float)/255
    va=np.array(Image.open(src/'moving_valid.tif'))>0;vb=np.array(Image.open(src/'target_valid.tif'))>0
    with np.load(folder/'residual_inverse_map.npz') as z:field=z['displacement_xy']
    q=GRID+field.reshape(-1,2);warped=sample(a,q).reshape(512,512)
    vm=sample(va.astype(float),q,0).reshape(512,512)>.5;warped[~vm]=0
    Image.fromarray(np.uint8(np.clip(warped,0,1)*255)).save(folder/'proposed_moving.tif')
    Image.fromarray(np.uint8(vm)*255).save(folder/'proposed_valid.tif')
    mask=va&vb&vm
    measures=dict(group=g,common_pixels=int(mask.sum()),before_corr=corr(a[mask],b[mask]),after_corr=corr(warped[mask],b[mask]),
        note='Secondary whole-overlap intensity check, not independent anatomical validation; same pixel mask before and after')
    metrics.append(measures)
    macro=f'''// Opens review copies only; this macro does not save labels or transforms.
open("{(folder/'manual_moving.tif').as_posix()}");
rename("Moving_{g}");
open("{(folder/'target.tif').as_posix()}");
rename("Target_{g}");
run("Big Warp");
'''
    (folder/'OPEN_PAIR_IN_FIJI.ijm').write_text(macro,encoding='utf-8')
    old=r['models'][0]['audit'];new=next(m for m in r['models'] if m['name']==r['selected'])['audit']
    err=f'{old["median_px"]:.2f} → {new["median_px"]:.2f} px ({old["n"]} reserved matches)' if old else 'Insufficient audit matches'
    html.append(f'''<section id="{g}"><h2>{g} <span class="badge">{r['selected']}</span></h2><p>{r['status']} · {err}</p><p><a href="{g}/matches.png">Inspect automatic matches</a> · <a href="{g}/OPEN_PAIR_IN_FIJI.ijm">Fiji helper macro</a> · <a href="{g}/result.json">Full measurements</a></p><button onclick="flip('{g}')">Blink right image: before / after</button><div class="pair"><div><p>Saved manual alignment</p><img src="{g}/before.png" alt="{g} before"></div><div><p id="label_{g}">Proposed output ({r['selected']})</p><img id="view_{g}" data-after="yes" src="{g}/after.png" alt="{g} after"></div></div></section>''')
html.append('''<script>function flip(g){const im=document.getElementById('view_'+g);const after=im.dataset.after!=='yes';im.dataset.after=after?'yes':'no';im.src=g+(after?'/after.png':'/before.png');document.getElementById('label_'+g).textContent=after?'Proposed output':'Saved manual alignment'}</script></body></html>''')
(OUT/'index.html').write_text('\n'.join(html),encoding='utf-8')
(OUT/'REPORT.md').write_text('\n'.join(lines),encoding='utf-8')
(OUT/'image_metrics.json').write_text(json.dumps(metrics,indent=2),encoding='utf-8')
verified=json.loads((OUT/'VERIFIED.json').read_text())
for path,h in verified['source_hashes'].items():assert hashlib.sha256(Path(path).read_bytes()).hexdigest()==h
verified['tests_passed']=35
verified['fiji_installed_bigwarp']='9.3.1'
