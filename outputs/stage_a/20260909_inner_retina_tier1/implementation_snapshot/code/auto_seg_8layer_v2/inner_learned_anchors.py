...
def run(args):
    ...
    if not (models / "training_complete.json").exists():
        raise ValueError("Finish all independent fold models before evaluating this control")
    metadata = permitted_records(m, partitions)
    permitted = {r["animal"] for r in metadata}
    complete = json.loads((models / "training_complete.json").read_text())
    ...
    write_json(out / "experiment.json", dict(priors=priors, protocol=fingerprint(models / "protocol.json"),
        ...
        final_test_used=False, production_adopted=False))
