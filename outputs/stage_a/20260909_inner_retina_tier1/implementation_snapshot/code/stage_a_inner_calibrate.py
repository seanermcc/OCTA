...
def run(args):
    ...
    if not (args.models / "training_complete.json").exists():
        raise ValueError("Complete all independent fold models before calibrating")
    complete = json.loads((args.models / "training_complete.json").read_text())
    ...
    write_json(out / "calibration_summary.json", dict(decoder=args.decoder,
        ...
        roles="Five train; one distinct calibration animal supplies entropy quantiles; a seventh animal supplies held-out coverage and error. Roles rotate deterministically.",
        evidence="Development cross-validation of the specified workflow. Final-test animals remain locked. Legacy surface-wide annotations and the small animal count limit generalisation.",
        quote_policy="Quote only the measured per-animal coverage/error ranges for this seven-animal study with b0510 sensitivity; no promised deployment coverage or universal quality score.",
        ...
        thresholds_selected_on_held_animal=False, validated=False, final_test_used=False))
