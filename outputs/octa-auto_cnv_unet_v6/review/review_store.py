...
class ReviewStore:
    def __init__(self,directory,scan,model='B_267',proposal_root=None):
        ...
        self.ratings={};self.review_events=[];self.active_error_ids=[];self.outputs={}
        ...
        if self.path.exists():
            ...
            self.ratings=data.get('model_ratings',{});self.review_events=data.get('review_events',[])
            self.active_error_ids=data.get('active_error_ids',[])
    ...
    def signature(self):
        return json.dumps(dict(regions=[r.record() for r in self.regions if self.persistent(r)],scan_review=self.scan_review,
            ratings=self.ratings,review_events=self.review_events,active_error_ids=self.active_error_ids),sort_keys=True)
    ...
    def output(self,model):
        if model in self.outputs:return self.outputs[model]
        path=self.proposal_root/model/f'{self.scan.scan_id}.npz'
        ...
        a=npz(path);labels=a['candidate_labels']
        assert labels.shape==self.scan.native_shape and np.array_equal(labels>0,a['mask'])
        a['source']=dict(path=str(path),sha256=disk_fingerprint(path),model=model)
        for value in a.values():
            if isinstance(value,np.ndarray):value.flags.writeable=False
        self.outputs[model]=a
        return a

    def switch(self,model):
        self.regions=[r for r in self.regions if self.persistent(r)]
        self.model=model;a=self.output(model);labels=a['candidate_labels']
        self.seed_path=a['source']['path'];self.seed_sha256=a['source']['sha256']
        self.proposal_mask=a['mask'];self.proposal_score=a['score']
    ...
    def event(self,action,**details):
        event=dict(id=uuid.uuid4().hex,at=datetime.now(timezone.utc).isoformat(),action=action,
            acquisition=self.scan.scan_id,**copy.deepcopy(details))
        self.review_events.append(event)
        return event

    def rate(self,key,value,displayed,reason='explicit checkbox'):
        assert value in ('acceptable','unacceptable',None)
        ex,seed=key.split('_');before=copy.deepcopy(self.ratings.get(key))
        entry=dict(value=value,at=datetime.now(timezone.utc).isoformat(),acquisition=self.scan.scan_id,
            experiment=ex,seed=int(seed),original_output=self.output(key)['source'],
            displayed_outputs=displayed,reason=reason)
        self.ratings[key]=entry
        self.event('model rating',key=key,before=before,after=entry)
        self.invalidate_complete()

    def error(self,kind,region,keys,displayed):
        assert kind in ('false suggestion','missed lesion','gross outline correction')
        original=self.output(region.id.split(':')[0])['candidate_labels']==int(region.id.split(':')[1]) if region.seed_ids else None
        footprints=dict(native_shape=list(self.scan.native_shape),axis_order='B-scan,A-line',
            human_runs=region.record()['runs'] if kind!='false suggestion' else [],
            original_runs=encode(original) if original is not None else [])
        for key in keys:
            ex,seed=key.split('_')
            event=self.event(kind,region_id=region.id,suggestion_id=region.id if region.seed_ids else None,
                acquisition_model=key,experiment=ex,seed=int(seed),original_output=self.output(key)['source'],
                displayed_outputs=displayed,footprints=footprints,training_label=None,
                interpretation='Review evidence only; no definitive negative or whole-field labels')
            self.active_error_ids.append(event['id'])
            self.rate(key,'unacceptable',displayed,reason=kind)
    ...
    def save(self,*unused):
        ...
        try:
            ...
            data=dict(format='2-original-prediction-review',reviewer_version=VERSION,scan_id=self.scan.scan_id,
                ...
                model_ratings=self.ratings,review_events=self.review_events,active_error_ids=self.active_error_ids,
                rating_semantics='Each acquisition/experiment/seed rates original unedited outputs; unset means unreviewed.',
            ...
            if self.path.exists():
                # exFAT allocates 256 KiB for even a tiny revision. Bundle new history
                # losslessly, while retaining all original legacy history files.
                history=dest(self.path.parent/'history'/f'{self.path.stem}_revisions.zip')
                entry=f'r{self.revision:06d}.json';previous={}
                if history.exists():
                    with zipfile.ZipFile(history) as z:previous={n:z.read(n) for n in z.namelist()}
                if entry not in previous:
                    previous[entry]=self.path.read_bytes();temp=history.with_suffix('.tmp.zip')
                    try:
                        with zipfile.ZipFile(temp,'w',compression=zipfile.ZIP_DEFLATED) as z:
                            for name,raw in previous.items():z.writestr(name,raw)
                        with zipfile.ZipFile(temp) as z:
                            for name,raw in previous.items():assert z.read(name)==raw
                        temp.replace(history)
                    finally:temp.unlink(missing_ok=True)
            write(self.path,data);self.revision+=1;self.disk_hash=disk_fingerprint(self.path);self.saved_signature=self.signature()
            return True
