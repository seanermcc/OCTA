...
class Window(W.QMainWindow):
    ...
    def __init__(self,start=0,manual=False,autoload=True):
        ...
        nav.addAction('Previous',lambda:self.step_scan(-1))
        ...
        nav.addAction('Next',lambda:self.step_scan(1));nav.addSeparator()
    ...
    def step_scan(self,step):
        self.load_scan(self.index+step)
    ...
    def render_maps(self):
        ...
        for canvas in (self.structural,self.second):
            ...
            canvas.review_items=[]
            for i,r in visible:
                if r.seed_ids and not self.store.persistent(r):continue
...
if __name__=='__main__':
    raise SystemExit('Use OPEN_CNV_REVIEW.cmd for the current original-prediction reviewer.')
