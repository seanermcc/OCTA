...
def encode(mask):
    # Preserve the exact row-major half-open run format.
    padded=np.pad(np.asarray(mask,dtype=bool),((0,0),(1,1)))
    rows,edges=np.nonzero(padded[:,1:] != padded[:,:-1])
    return np.column_stack((rows[::2],edges[::2],edges[1::2])).tolist()
