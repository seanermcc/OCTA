...
class Window(W.QMainWindow):
    ...
    def __init__(self,start=None,review_directory=None,synthetic=False,autoload=True):
        super().__init__();self.setWindowTitle('CNV v9 · Final correction · 14 flagged acquisitions');self.resize(1510,980)
        ...
        self.progress_cache={};self._bscan_key=None;self._band_items=[];self.export_process=None
        ...
        layout.addWidget(W.QLabel('Final assessment flags and source notes (preserved)'))
        self.review_notes=W.QPlainTextEdit();self.review_notes.setReadOnly(True);self.review_notes.setMaximumHeight(85);self.review_notes.setPlaceholderText("No gallery review notes for this sample");layout.addWidget(self.review_notes)
        ...
        legend=W.QLabel('<span style="color:#269d55">Green: kept CNV</span> · <span style="color:#c77515">Orange: Selected footprint / new draft</span> · <span style="color:#199aa8">Cyan: assessment reference</span> · <span style="color:#985aca">Purple: unsure / excluded</span> · Gray: removed · Gold: Model 3 including hidden candidates');legend.setWordWrap(True);layout.addWidget(legend)
        ...
        self.prediction=W.QCheckBox('Reference: all Model 3 candidates');sv.addWidget(self.prediction)
        self.historical=W.QCheckBox('Reference: assessment versions');sv.addWidget(self.historical)
        ...
        self.preview_timer=Q.QTimer(self);self.preview_timer.setSingleShot(True);self.preview_timer.setInterval(33);self.preview_timer.timeout.connect(self.render)
    ...
    def toolbar(self):
        ...
        filters.addAction('Export final dataset + sizes',self.export_sizes)
    ...
    def loaded(self,scan):
        ...
        a=scan.acquisition
        self.review_notes.setPlainText(a.get("review_notes", ""))
        ...
        self.save_session();self.statusBar().showMessage('Ready · Flagged final footprints require your correction; saved edits resume automatically');self.ready.emit()
    ...
    def stroke_preview(self,stroke):
        r=self.current()
        if r is None:return
        old=decode(r['runs']);self.preview_mask=old&~stroke if self.operation=='erase' else old|stroke
    ...
    def stroke(self,stroke):
        ...
        self.preview_timer.stop();self.preview_mask=None;self.store.set_region(self.selected,mask=mask);self.changed()
    ...
    def navigate(self,row,col):
        ...
        self.render(overlays_changed=False)

    def render(self,*args,overlays_changed=True):
        if not self.scan:return
        overlays=self.displayed()
        for canvas in (self.structural,self.octa):
            if overlays_changed:canvas.overlays(overlays)
            canvas.cursor(self.row,self.col)
        key=(id(self.scan),self.row)
        if key!=self._bscan_key:
            self.bscan.image(self.scan.images[self.row]);self._band_items=[];self._bscan_key=key
        scene=self.bscan.scene();height=self.scan.images.shape[1];self.band_geometry=[]
        for item in self._band_items:scene.removeItem(item)
        self._band_items=[]
        for mask,color,selected in overlays:
            ...
            for left,right in intervals(mask[self.row]):
                item=scene.addRect(left,0,right-left,height,pen,G.QBrush(c));item.setZValue(4);self._band_items.append(item)
                item=scene.addRect(left,0,right-left,4,G.QPen(Qt.PenStyle.NoPen),G.QBrush(G.QColor(color)));item.setZValue(5);self._band_items.append(item)
                self.band_geometry.append((left,right,color,selected))
        pen=G.QPen(G.QColor('#70dcf8'),1,Qt.PenStyle.DotLine);pen.setCosmetic(True);item=scene.addLine(self.col+.5,0,self.col+.5,height,pen);item.setZValue(7);self._band_items.append(item)
    ...
    def update_progress(self):
        p=progress(self.queue,self.review_directory,self.store,cache=self.progress_cache)
        ...
        self.coverage.setText('Per animal, positive / negative: '+ '   ·   '.join(f'{a}: {c["positive"]}/{c["negative"]}' for a,c in p['per_animal'].items())+('   ALL QUEUED CASES REVIEWED — use Export final dataset + sizes to refresh the report.' if p['target_reached'] else ''))
    ...
    def save(self):
        ...
        try:
            ...
            if saved and not self.synthetic:
                self.statusBar().showMessage('Reviews saved. Size reports refresh separately with Export final dataset + sizes.')
            return True
        except Exception as exc:
        ...
        try:
            ...
            if self.save():
                ...
                if p['target_reached'] and not self.milestone_shown:
                    self.milestone_shown=True;W.QMessageBox.information(self,'All queued cases reviewed',f'{p["positive"]} positive images and {p["negative"]} negative images saved. Use Export final dataset + sizes to refresh reports/quantification and dataset. No training starts automatically.')
    ...
    def export_sizes(self):
        ...
        if self.export_process is not None:
            self.statusBar().showMessage('Size export is already running; editing remains available.');return
        process=Q.QProcess(self);self.export_process=process
        process.setWorkingDirectory(str(HERE))
        process.setProgram(sys.executable);process.setArguments(['-B',str(HERE/'measure.py')])
        process.finished.connect(self.export_finished)
        process.errorOccurred.connect(lambda error:self.export_failed(process.errorString()) if error==Q.QProcess.ProcessError.FailedToStart else None)
        process.start();self.statusBar().showMessage('Exporting confirmed sizes in the background…')
    ...
    def export_failed(self,message):
        ...
        self.statusBar().showMessage('Size export failed; reviews are saved: '+message)

    def export_finished(self,code,status):
        process=self.export_process
        if process is None:return
        error=bytes(process.readAllStandardError()).decode(errors='replace')
        if code or status!=Q.QProcess.ExitStatus.NormalExit:
            self.export_failed(error[-500:] or 'Export interrupted');return
        self.export_process=None;process.deleteLater()
        self.statusBar().showMessage('Confirmed size reports exported to reports/quantification and dataset.')
    ...
    def closeEvent(self,event):
        if self.save():
            self.autosave.stop();self.preview_timer.stop();self.cache.close()
            if self.export_process is not None:
                self.export_process.kill()
                if self.export_process is not None:self.export_process.waitForFinished(1000)
            event.accept()
        else:event.ignore()
