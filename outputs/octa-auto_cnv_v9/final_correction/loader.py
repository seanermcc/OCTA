"""Native optical provider; new caches stay inside the correction directory."""
from common import *
...
def context_overlays(a,kind):
    ...
    if kind=='historical' and a.get('assessment_references'):
        return [dict(mask=decode(r['runs']),color='#46dbea' if r['source']=='manual' else '#e3bc57',label=r['source_label'],source=dict(kind='final_assessment_reference',entry_id=r['entry_id'],token=r['token'])) for r in a['assessment_references']]
    if kind=='prediction':
        from proposals import model_arrays
        z,doc=model_arrays(a)
        return [dict(mask=z['raw_mask'].astype(bool),color='#e3bc57',label='Model 3 including hidden candidates',
            source=dict(kind='model3_raw_reference',record=doc))]
