...
class Handler(SimpleHTTPRequestHandler):
 ...
 def do_GET(self):
  url=urlparse(self.path)
  if url.path=='/health':
   self.json_response(dict(app='octa-cnv-v9-model3-gallery',root=str(HERE),pid=os.getpid(),review_api=1));return
  if url.path in ('/reviews','/reviews/export','/reviews/model3-queue'):
   try:self.json_response(REVIEWS.queue() if url.path.endswith('model3-queue') else REVIEWS.all())
   except ReviewConflict as exc:self.json_response(dict(error=str(exc)),409)
   return
  if url.path=='/bscan':
   try:
...
def main():
 ...
 parser=argparse.ArgumentParser();parser.add_argument('--port',type=int,default=8803);parser.add_argument('--open',action='store_true');parser.add_argument('--fixture',action='store_true');args=parser.parse_args()
