...
class Window(W.QMainWindow):
    ...
    def update_progress(self):
        ...
        self.coverage.setText('Per animal, positive / negative: '+ '   ·   '.join(f'{a}: {c["positive"]}/{c["negative"]}' for a,c in p['per_animal'].items())+('   ALL 57 REVIEWED — confirmed size report is ready.' if p['target_reached'] else ''))
        ...
        try:
            ...
            if self.save():
                ...
                if p['target_reached'] and not self.milestone_shown:
                    self.milestone_shown=True;W.QMessageBox.information(self,'All 57 reviewed',f'{p["positive"]} positive images and {p["negative"]} negative images saved. Confirmed sizes are in reports/quantification. No training starts automatically.')
    ...
    def export_sizes(self):
        ...
        try:
            from measure import run
            summary=run()
            W.QMessageBox.information(self,'Confirmed CNV sizes',f'{summary["confirmed_images"]}/57 images confirmed; {summary["confirmed_lesion_regions"]} CNV regions.\nSaved lesions.csv, scans.csv, summary.json and derived training_manifest.json in:\n'+str(HERE/'reports/quantification'))
