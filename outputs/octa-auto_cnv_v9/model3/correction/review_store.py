...
def progress(acquisitions, directory=None, active=None, verify_synthetic=False, cache=None):
    ...
    for a in acquisitions:
        ...
        try:
            if active and active.acquisition['scan_id']==a['scan_id'] and (not active.synthetic or verify_synthetic):
                ...
                stat=path.stat()
                stamp=(stat.st_mtime_ns,stat.st_size,stat.st_ctime_ns,key,verify_synthetic)
                cached=cache.get(str(path)) if cache is not None else None
                if cached and cached[0]==stamp:
                    kind=cached[1];counts[kind]+=1
                ...
                kind=completion_kind(state,tuple(a.get('native_shape',[512,512]))[:2])
                after=path.stat()
                if (after.st_mtime_ns,after.st_size,after.st_ctime_ns)!=stamp[:3]:raise ValueError('Record changed during validation; retry')
                if cache is not None:cache[str(path)]=(stamp,kind)
            else:continue
