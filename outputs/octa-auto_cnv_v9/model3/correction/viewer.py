...
class Window(W.QMainWindow):
    ...
    def __init__(self,start=None,review_directory=None,synthetic=False,autoload=True):
        super().__init__();self.setWindowTitle('CNV v9 Model 3 · 57 flagged scans · correction editor');self.resize(1510,980)
        ...
        layout.addWidget(W.QLabel('Original gallery review notes (preserved)'))
        ...
        legend=W.QLabel('<span style="color:#269d55">Green: kept CNV</span> · <span style="color:#c77515">Orange: Model 3 draft / new draft</span> · <span style="color:#199aa8">Cyan: historical reference</span> · <span style="color:#985aca">Purple: unsure / excluded</span> · Gray: removed · Gold: Model 3 including hidden candidates');legend.setWordWrap(True);layout.addWidget(legend)
    ...
    def loaded(self,scan):
        ...
        self.save_session();self.statusBar().showMessage('Ready · Model 3 proposals require your review; saved corrections resume automatically');self.ready.emit()
    ...
    def update_progress(self):
        ...
        self.coverage.setText('Per animal, positive / negative: '+ '   ·   '.join(f'{a}: {c["positive"]}/{c["negative"]}' for a,c in p['per_animal'].items())+('   ALL 57 REVIEWED — use Export confirmed sizes to refresh the report.' if p['target_reached'] else ''))
    ...
    def save(self):
        ...
        try:
            ...
            if saved and not self.synthetic:
                self.statusBar().showMessage('Reviews saved. Size reports refresh separately with Export confirmed sizes.')
            return True
        except Exception as exc:
        ...
        try:
            ...
            if self.save():
                ...
                if p['target_reached'] and not self.milestone_shown:
                    self.milestone_shown=True;W.QMessageBox.information(self,'All 57 reviewed',f'{p["positive"]} positive images and {p["negative"]} negative images saved. Use Export confirmed sizes to refresh reports/quantification. No training starts automatically.')
    ...
    def export_finished(self,code,status):
        ...
        self.statusBar().showMessage('Confirmed size reports exported to reports/quantification.')
