...
class Window(W.QMainWindow):
    ...
    def __init__(self,start=None,review_directory=None,synthetic=False,autoload=True):
        super().__init__();self.setWindowTitle('CNV footprint review · v7');self.resize(1510,980)
        ...
        legend=W.QLabel('<span style="color:#269d55">Green: human-confirmed CNV</span> · <span style="color:#c77515">Orange: draft</span> · <span style="color:#199aa8">Cyan: historical reference</span> · <span style="color:#985aca">Purple: unsure / excluded</span> · Gray: removed · Gold: unreviewed v6 prediction');legend.setWordWrap(True);layout.addWidget(legend)
        ...
        self.prediction=W.QCheckBox('Reference: original v6 C / seed 267');sv.addWidget(self.prediction)
        ...
        self.context_note=W.QLabel('References are read-only, hidden by default. Seed 267 is context, not a validated best seed.');self.context_note.setWordWrap(True);sv.addWidget(self.context_note)
    ...
    def loaded(self,scan):
        ...
        self.save_session();self.statusBar().showMessage('Ready · no annotation is created by browsing');self.ready.emit()
    ...
    def refresh(self):
        ...
        for i,r in enumerate(self.store.state['regions']):
            self.region_list.addItem(f'{i+1}. {r["state"].title()} · {sum(b-a for _,a,b in r["runs"])} px')
    ...
    def update_progress(self):
        ...
        self.progress_label.setText(prefix+f'Confirmed CNV images: {p["positive"]} / 30     Confirmed no-CNV images: {p["negative"]}     Draft: {p["draft"]} · Deferred: {p["deferred"]}     Queue {self.index+1} / {len(self.queue)}')
        self.coverage.setText('Per animal, positive / negative: '+ '   ·   '.join(f'{a}: {c["positive"]}/{c["negative"]}' for a,c in p['per_animal'].items())+('   COLLECTION TARGET REACHED — save and stop, or continue reviewing.' if p['target_reached'] else ''))
    ...
    def save(self):
        ...
        try:
            ...
            self.store.save();self.save_session();self.refresh();return True
        except Exception as exc:
        ...
        try:
            ...
            if self.save():
                ...
                if p['target_reached'] and not self.milestone_shown:
                    self.milestone_shown=True;W.QMessageBox.information(self,'Collection target reached',f'{p["positive"]} positive images and {p["negative"]} negative images safely saved. This is a collection milestone, not model validation. You may stop or continue; no training starts.')
