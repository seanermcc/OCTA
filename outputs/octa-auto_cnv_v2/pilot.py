...
from common import *
from algorithm import detect, geometry, quantify, VARIANTS, grow
...
def figure(sid, arrays, output):
    ...
    fig.suptitle(sid+'\nocta-auto_cnv_v2 · experimental; no visit registration', fontsize=13, y=.98)
...
def process(visit):
    ...
    path=volume_path(sid); base=npz(path/'v1_matched_baseline.npz'); g=npz(path/'geometry.npz')
    images=np.load(path/'images.npy',mmap_mode='r')
    arrays.update(geometry(base['raw_position_branch'],int(g['label_offset']),images.shape[1]))
    from eight_surface.config import SURFACE_NAMES
    arrays['surface_names']=np.array(SURFACE_NAMES)
    arrays['automatic_surfaces_crop_px']=base['raw_position_branch']-int(g['label_offset'])
    ...
    save_npz(HERE/'proposals'/f'{sid}.npz',core=maps['core'],proposal_mask=maps['core'],candidate_labels=arrays['candidate_labels'],
