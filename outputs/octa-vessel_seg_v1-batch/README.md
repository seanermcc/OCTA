...
Refresh `index.html` to show ONH present and Vessel issue checkboxes below each
thumbnail. These are independent triage flags, not annotation approval.
Selections persist in this browser's local storage. Use Export review queue
to download `octa-vessel-v1-review-queue.json` for backup and later GUI queue
loading. Import saved queue merges a previous export. The Show filter can
display only flagged scans. No vessel/ONH mask or human label is modified.
