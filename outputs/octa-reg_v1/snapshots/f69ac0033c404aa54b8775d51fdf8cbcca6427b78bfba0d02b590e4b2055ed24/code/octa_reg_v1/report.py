...
def report(out,assets=True):
    out=Path(out);infos={i['scan_id']:i for i in read(out/'inventory.json')}
    pairs=[]
    for ab in read(out/'candidate_pairs.json'):
        p=read(out/'pairs'/(ab['scan_a']+'__'+ab['scan_b']+'.json'))
        checksum=p.pop('artifact_digest',None)
        if checksum!=digest(p):raise ValueError('Pair artifact digest mismatch: '+p['pair_id'])
        p['artifact_digest']=checksum;pairs.append(p)
