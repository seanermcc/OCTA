...
def code_hashes():
    ...
    paths=list(folder.glob('*.py'))+[folder.parent/'control_map_v1/geometry.py',folder.parent/'control_map_v1/__init__.py']
    ...
    try:
        ...
        def compute(ab):
            ...
            if ip.exists():
                try:
                    old=read(ip)
                    if old['signature']==sig and old['status']!='implementation_failure':return old,True
        ...
        with ThreadPoolExecutor(max_workers=cfg['workers']) as pool:
            for res,resumed in pool.map(compute,candidates):
                if not resumed:write(out/'pairs'/(res['pair_id']+'.json'),res)
